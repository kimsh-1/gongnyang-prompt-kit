...
2. 카테고리/컷타입이 잡히면 `references/category-patterns.md`(C1~C12)를 본다. 포맷 라우팅(아래 §포맷 A/포맷 B)으로 A/B를 정한다. 무드 요청("있어보이게"·"럭셔리하게"·"영화처럼")은 `references/look-presets.md`에서 프리셋 1개를 골라 드롭인. **홍보판촉물·홍보이미지·브랜드 포스터 요청("홍보물 뽑아줘"·"판촉물 느낌"·"디자인 잘된 포스터")은 `references/promo-graphics.md`(P1~P8)에서 패턴 1개를 골라 레이아웃 골격으로** — 카드뉴스 밀도 문법으로 홍보물을 만들면 미감이 죽는다(C7 미감 라우팅 2모드). 시안 다변화·양산 컨셉·"차별화되게"·"컨셉부터" 요청은 `references/concept-axes.md`의 변수 축(사조 M·몸 반응 R·모순쌍 X·타이포 아트 T·컬러 번역)에서 축 1개를 골라 변주.
...
- **`references/promo-graphics.md`** — 홍보판촉물 그래픽 문법 P1~P8(타이포-마스크·타이포-환경·오버사이즈 크롭+오클루전·컬러 블로킹 캠페인 시스템·메타 UI 디바이스·스트리트 콜라주·에디토리얼 회전축·모노크롬 스테이징) + 공통 마감 디바이스. 레퍼런스 14장(2026-07-12) 증류.
